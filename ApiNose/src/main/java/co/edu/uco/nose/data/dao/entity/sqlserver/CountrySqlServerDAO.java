...
public final class CountrySqlServerDAO extends SqlConnection implements CountryDAO {
    ...
    public CountrySqlServerDAO(final java.sql.Connection connection) {
        super(connection);
    }
    ...
    public List<CountryEntity> finAll() {
        ...
        try (PreparedStatement ps = getConnection().prepareStatement(sql);
            ...
            while (rs.next()) {
                results.add(mapResultSetToCountryEntity(rs));
            }
            return results;
    ...
    public List<CountryEntity> findByfilter(final CountryEntity filterEntity) {
        ...
        sb.append("SELECT id, name FROM country WHERE 1=1");
    ...
    public CountryEntity findById(final UUID id) {
        ...
        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return mapResultSetToCountryEntity(rs);
                }
            }
            return null;
        } catch (final SQLException exception) {
    ...
    private CountryEntity mapResultSetToCountryEntity(final ResultSet rs) throws SQLException {
