...
public final class CitySqlServerDAO extends SqlConnection implements CityDAO {
	...
	public List<CityEntity> findByfilter(CityEntity filterEntity) {
		...
		try (var preparedStatement = this.getConnection().prepareStatement(CitySql.FIND_BY_FILTER)) {
			...
			if (filterEntity.getDepartment() != null && filterEntity.getDepartment().getId() != null) {
				preparedStatement.setObject(index++, filterEntity.getDepartment().getId());
			}
